    ...
    pub fn min_difficulty(job_difficulty: Vec<i32>, d: i32) -> i32 {
        ...
        for day in 1..d {
            for task in (day + 1)..n {
                ...
                let min_day_before = dp[day - 1][(prev_big.max(day - 1))..=(task - 1)]
                    .iter()
                    .fold(i32::MAX - 1000000, |a, e| a.min(*e))
                    + job_difficulty[task];
...
mod test {
    ...
    fn test6() {
        assert_eq!(
            Solution::min_difficulty(
                vec![186, 398, 479, 206, 885, 423, 805, 112, 925, 656, 16, 932, 740, 292, 671, 360],
                4
            ),
            1803
        );
    ...
    fn test7() {
        assert_eq!(
            Solution::min_difficulty(
                vec![3, 7, 9, 4, 14, 8, 13, 2, 15, 10, 1, 16, 12, 5, 11, 6],
                4
            ),
            32
        );
    ...
    fn test8() {
        assert_eq!(
            Solution::min_difficulty(vec![10, 1, 16, 12, 5, 11, 6], 4),
            33
        );
    }
}
